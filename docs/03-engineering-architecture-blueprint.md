...
| -------------------------------------------- | ----------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `user`, `session`, `account`, `verification` | Autenticação (Better Auth)          | `user`: id, name, email (único), email_verified, image, **`role`** (enum ADMIN/EDITOR/AUTHOR), `two_factor_enabled`, `disabled_at`. As demais seguem a biblioteca                                                                                                                                           |
...
**Pontos a confirmar na implementação [ABERTA, técnicos]:** nome exato da opção que desliga o cadastro público; algoritmo de hash padrão da biblioteca; se o rate limit do login usa armazenamento em banco (a doc lista `storage: "database"` como opção, o padrão em memória não serve em ambiente sem estado compartilhado).
...
| Autenticação com 2FA e cadastro fechado                                      |    ☐     | Fase do CMS                       |
| Autorização por `can(...)` em toda operação; testes de IDOR                  |    ☐     | Fase do CMS                       |
| Validação Zod em toda entrada; limites de tamanho                            |    ◐     | `env.ts` apenas                   |
| Rate limit (login, contato, newsletter, upload)                              |    ☐     | Fases CMS/Leads                   |
...
| ---- | ---------------- | ----------------------------------------------------------------------------------------------------- | ----------------------------------------------------------- | ------------------------------------------------------------ |
| 1    | Fundação técnica | TS, lint, formatação, env, erros, logger, testes; **cliente de banco e configuração de autenticação** | **Parcial**: falta banco e auth (Prompt 4 §9). Chamada "1b" | `npm run check` verde + conexão do app ao banco por `dm_app` |
...
| 3    | Banco e domínio  | Schema Drizzle, migrations, roles, seeds `[DEMO]`, módulos de domínio com testes de integração        | Não iniciada                                                | Migrations reproduzíveis; constraints testadas               |
