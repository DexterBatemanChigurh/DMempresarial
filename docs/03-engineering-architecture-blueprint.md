...
**Implementado (Fase 4, incremento 2, 21/09/2026) e ajuste em relação ao texto acima:** a renderização **não** usa o renderizador estático do Tiptap: usa um renderizador React próprio (`components/content/rich-text.tsx`) sobre o JSON já validado. Motivo: controle total do Design System (citação, destaque, links, imagens) e uma dependência a menos no site público. O ganho de segurança é o mesmo ou maior: não existe HTML montado como texto em lugar nenhum (um teste proíbe `dangerouslySetInnerHTML`, `innerHTML` e afins em todo o código), o texto é escapado pelo React e o documento é **validado de novo na leitura**. O Tiptap ficou só no editor do painel.

- **Validador por lista de permissão** (`lib/rich-text/validate.ts`): reconstrói o documento com objetos novos, recusa (não "limpa") o que não está na lista, limita profundidade, número de nós e tamanho, recusa `__proto__`/`constructor`, e nunca devolve o conteúdo hostil nas mensagens. Links só `http(s)` com host, `mailto`, `tel`, caminho interno e âncora; `@` na parte de autoridade, `javascript:`, `data:`, `//` e espaços são recusados. Imagens só por id de mídia.
- **Editor e servidor concordam**: o schema do editor é comparado com a lista de permissão em teste (nenhum elemento a mais, nenhum atributo desconhecido) e o link que o editor cria é aceito pelo servidor. Esse teste nasceu de um defeito real achado no navegador: o Tiptap 3 emite `title` no link e o validador o recusava.
- **`prepareRichBody`** é a porta de entrada obrigatória de todo salvamento: normaliza e deriva o texto plano e o tempo de leitura.
...
- **Redirecionamentos (Prompt 3 §69):** tabela `redirects`. **Revisado na Fase 8** [VERIFICADO empiricamente]: a resolução acontece no **Proxy** (`src/proxy.ts`), não na página. Testado e confirmado: sob Cache Components/PPR, um `permanentRedirect()` chamado a partir de dado de banco (dentro do trecho adiado/streaming de `/blog/[slug]`) não produz um 301/308 HTTP de verdade — o Next manda 200 e resolve a navegação por JS no cliente (comportamento documentado do próprio `permanentRedirect` para "streaming context"). O Proxy roda em runtime Node.js por padrão no Next 16 [VERIFICADO em `file-conventions/proxy`] e decide antes da página renderizar, então consulta `redirects` só para `/blog/<slug>` (leitura pontual por chave, indexada) e devolve um redirecionamento HTTP real quando existe. Ver ADR-016.
