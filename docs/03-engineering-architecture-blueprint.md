...
| 4    | CMS              | Login em duas etapas (TOTP + códigos de backup), Proxy com CSP de nonce e request id, painel com guarda de sessão e 2FA obrigatório, primeiro admin por script. **Falta:** editor Tiptap, uploads, CRUD de artigos/taxonomia/especialistas/soluções/páginas, usuários, preview, cron de agendamento, visualização da auditoria | **Em andamento (incremento 1 de 5 concluído em 21/09/2026)**                                                                                          | Matriz de permissões 100% testada, incluindo IDOR e upload   |
