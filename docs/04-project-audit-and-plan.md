...
| 3     | ✔      | Design System: tokens em duas camadas (`globals.css`), fontes por `next/font` (149 KB medidos), primitivos de UI (Container, Section, SectionLabel, Heading, Text, Button, IconButton, TextLink, TextField/TextareaField/SelectField/CheckboxField, FormMessage, SkipLink), página de referência `/design-system` (só fora de produção), testes de contraste e de acessibilidade dos componentes, verificação no Chrome real (fontes, cores, 320 a 1920 px sem estouro, foco por teclado) |
...
Achados do passo 3 (corrigidos ou registrados): as fontes pesavam 272 KB com o eixo `opsz`, o dobro do orçamento; removido (149 KB, decisão registrada no Blueprint 2, seção 07). **Adiado para a Fase 5:** o componente `Figure` (não há nenhuma imagem real ainda; nasce com o primeiro conteúdo) e o Header/Footer/MobileMenu (dependem do conteúdo e da marca, L-02).

| Passo | Estado | O que ficou pronto e verificado                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| ----- | ------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 4     | ✔      | **Fase 3, banco e domínio:** 23 tabelas com constraints (slug, estado × datas, tamanhos, e-mail, mídia, redirecionamento), migrations `0000` a `0004`, `audit_logs` só-inserção, busca sem acento, seed de dados confirmados, regras de domínio puras (máquina de estados, pré-requisitos de publicação de artigo, solução e especialista), serviços `transitionPost` e `changePostSlug` (transação, `FOR UPDATE`, bloqueio otimista, auditoria atômica, redirecionamento 301) e leitura pública que só enxerga `PUBLISHED`. 214 testes unitários e 77 de integração |

Verificações da Fase 3: quatro mutações nas defesas críticas (vazamento de rascunho, esconder a existência do objeto, redirecionamento, concorrência) derrubam 10 testes; restauradas, os 77 passam. Decisão pendente das duas perguntas do passo 4 resolvida pelo padrão recomendado: **leads visíveis só a ADMIN** e **2FA obrigatório antes de leads reais** (a implementar na Fase 4).

**Banco remoto (Neon), fornecido pelo usuário:** vazio no primeiro contato (0 tabelas), PostgreSQL 18.6, região `us-east-2` (Ohio), aceita `citext` e `unaccent`. Credenciais só em `.env.neon` (ignorado pelo git). A senha do dono foi colada no chat: **rotacionar** ao terminar. O ambiente local usa PostgreSQL 17; a paridade de versão (18) fica como decisão de infraestrutura.

**Aplicado no Neon em 21/09/2026** (`npm run db:setup:remote`): 23 tabelas, 12 enums, 5 migrations, 82 constraints, seed confirmado. Verificado como `dm_app` pelo pooler: não cria tabela, não altera esquema, não apaga/altera/trunca a auditoria (só insere), busca sem acento funciona. A suíte de integração completa foi executada em **PostgreSQL 17 e em 18** (77/77 nas duas). Diferença entre versões achada assim: `ON DELETE RESTRICT` responde SQLSTATE `23503` no PG 17 e `23001` no PG 18; código de aplicação que traduza esse erro deve aceitar os dois.

| Passo | Estado                      | O que ficou pronto e verificado                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ----- | --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 5     | ◐ Fase 4, incremento 1 de 5 | **Acesso e segurança do painel:** `proxy.ts` (request id, CSP com nonce só em `/admin`, redirecionamento otimista), 2FA TOTP obrigatório para ADMIN/EDITOR, telas de login, verificação, segurança (QR, backups, troca de senha) e painel inicial, primeiro admin por script. 238 unitários e 89 de integração. **Verificado num Chrome real (23 passos, 0 violações de CSP):** proteção do `/admin`, login errado com mensagem genérica, 2FA obrigatório cadastrado com QR e backups, logout, login em duas etapas, código errado recusado |

Faltam na Fase 4 (incrementos 2 a 5): motor de texto rico (Tiptap com validação por lista de permissão no servidor), mídia (interface de storage, pipeline de upload), CRUD de conteúdo (artigos com estados e preview, categorias e tags, especialistas, soluções, páginas, configurações), usuários e visualização da auditoria, cron de agendamento.

Achados do incremento: o `typecheck` passa a rodar `next typegen` antes do `tsc`, porque os tipos de rotas gerados ficam velhos ao criar layouts novos; o navegador pede `/favicon.ico` e recebe 404 (sem logotipo ainda, L-02).

| Passo | Estado                      | O que ficou pronto e verificado                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| ----- | --------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 6     | ◐ Fase 4, incremento 2 de 5 | **Motor de texto rico:** validador por lista de permissão (corpus de vetores de XSS, poluição de protótipo, bombas de profundidade e tamanho), regra de links, renderizador React sem HTML bruto (com teste que proíbe `innerHTML` e afins), `prepareRichBody`, extensões do Tiptap com schema comparado à lista, editor acessível e página de demonstração fora de produção. 336 unitários. **Verificado num Chrome real (30 passos, 0 erros de console):** colar HTML malicioso (script, onerror, onclick, iframe, svg, tabela, `javascript:`) deixa só parágrafos, nenhum script executa, nenhum diálogo abre |

Defeitos reais achados no navegador (corrigidos e cobertos por teste): o editor ficava preso no esqueleto porque o `useEditorState` demora a responder; todo link do editor seria recusado ao salvar porque o Tiptap 3 emite `title`; o botão de destaque não funcionava dentro de listas (e o `lift` não desfazia). Decisão registrada: renderizador próprio em vez do renderizador estático do Tiptap (Blueprint 3, parte 9).

Faltam na Fase 4 (incrementos 3 a 5): mídia (interface de storage, upload seguro), CRUD de conteúdo com estados e preview, usuários, auditoria e agendador.
