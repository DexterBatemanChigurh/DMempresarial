...
import {
  ...
  transitionSolutionAction,
  type SolutionMutated,
} from "@/app/admin/(painel)/solucoes/actions";
import { Button, FormMessage } from "@/components/ui";
import type { ActionResult } from "@/lib/result";
import type { SolutionStatus } from "@/features/catalog/application/solution-service";
...
const TRANSITION_LABEL: Record<SolutionStatus, string> = {
...
type Props = {
  ...
  version: number;
  targets: SolutionStatus[];
  canDelete: boolean;
};

export function SolutionStatusPanel({ solutionId, status, version, targets, canDelete }: Props) {
  ...
  >(async (prevState, formData) => {
    const result = await transitionSolutionAction(prevState, formData);
